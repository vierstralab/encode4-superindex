...
res_90 <- sapply(2:2000, function(k) {
  load(paste("meanSignal_topX/4527_mean_90_each/k", k, ".RData", sep="")) # res
  if(class(dhs) != "matrix") dhs <- t(as.matrix(dhs))
  dhs
});

res_70 <- sapply(2:2000, function(k) {
  load(paste("meanSignal_topX/4527_mean_70_each/k", k, ".RData", sep="")) # res
  if(class(dhs) != "matrix") dhs <- t(as.matrix(dhs))
  dhs
});

res_50 <- sapply(2:2000, function(k) {
  load(paste("meanSignal_topX/4527_mean_50_each/k", k, ".RData", sep="")) # res
  if(class(dhs) != "matrix") dhs <- t(as.matrix(dhs))
  dhs
});
...
#print(dat_new)
plotfile("/home/nasi4/public_html/encode4plus_figures/additional_DHS/meanSignal_topX/meanSignal_topX_Mean_allDHSs_topBottom", type="pdf")
par(cex=2,oma=c(0,3,0,0))
plot(x=1:n, y=data_90/(.1*total), type="l", lwd=2, xlim=c(0,2000), ylim=c(0, 1), col="black",
     xlab="Number of DNase-seq datasets", 
     ylab="Cumulative Sum of DHSs in topX% (%)")
title("Added DHSs for 4527 Index", cex.main=.75)
par(cex.lab = .5)
lines(1:n,  data_50/(.5*total), lwd=2, col="purple")
lines(1:n,  data_10/(.9*total), lwd=2, col="red")
#lines(1:n, dat_new_30$y, lwd=2, col="purple")
#lines(1:n, dat_new_10$y, lwd=2, col="blue")
legend('topleft', legend = c('Top 10%', 'Top 50%', 'Top 90%'), col = c('black', 'purple', 'red'), lty = 1, cex=.5, lwd=2)
dev.off()
